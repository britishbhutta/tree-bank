                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="fund_type" id="fund_type" class="form-control"
                                        ...
                                        <option value="Cheque" {{ $donation->fund_type == 'Cheque' ? 'selected' : '' }}>Cheque
                                        </option>
                                    </select>
                                </div>
                                        ...
                                        <div class="row mb-2 tree-row">
                                            <div class="col-md-4">
                                                ...
                                                <select name="trees[type_id][]" class="form-control tree-type" required
                                                    data-selected="{{ $tree['type_id'] }}">
                                                    <option value="">Select Type</option>
                                                    @foreach ($treetype as $t)
                                                        <option value="{{ $t->id }}"
                                                            {{ $t->id == $tree['type_id'] ? 'selected' : '' }}>
                                                            {{ $t->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-md-4">
                                                ...
                                                <select name="trees[tree_name_id][]" class="form-control tree-name" required
                                                    data-selected="{{ $tree['tree_name_id'] ?? '' }}">
...
.tree-item strong {
    ...
    const res = await fetch(
        `/admin/available-trees?project_id=${projectSelect.value}&donation_id=${donationId}`
    );
    ...
    cardsContainer.innerHTML = html;
            ...
            [typeSelect, flowSelect, projectSelect].forEach(el => el.addEventListener('change', loadAvailableTrees));
    ...
    `);
        ...
        document.addEventListener('DOMContentLoaded', () => {
            document.querySelectorAll('.tree-row').forEach(async row => {
                const typeSelect = row.querySelector('.tree-type');
                const nameSelect = row.querySelector('.tree-name');
                ...
                data.forEach(tree => {
                    ...
                    option.value = tree.id;
                    option.textContent = tree.name;
                    if (tree.id == selectedNameId) option.selected = true;
                    nameSelect.appendChild(option);
                });
            });
        });
